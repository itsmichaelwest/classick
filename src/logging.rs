...
//!    file under `%LOCALAPPDATA%\ipod-sync\logs\` (or platform equivalent).
...
///   `%LOCALAPPDATA%\ipod-sync\logs\core-{unix_ts}.log` (see
...
///
/// Layout: `<data_local_dir>/ipod-sync/logs/core-{unix_timestamp}.log`.
/// On Windows this resolves to `%LOCALAPPDATA%\ipod-sync\logs\`. Falls back
