...
fn initialized_fixture_is_ready_when_injected_validator_succeeds_without_writes() {
    ...
    let mut validated_mount = None;

    assert_classification_is_read_only(
        &mount,
        || {
            classify_device_readiness_with(mount.path(), |path| {
                validated_mount = Some(path.to_path_buf());
                true
            })
        },
        Some(DeviceReadiness::Ready),
    );
    assert_eq!(validated_mount.as_deref(), Some(mount.path()));
}
