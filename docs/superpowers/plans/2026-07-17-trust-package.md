...
- Produces in `db.rs`:
  ...
  - `pub fn restore_itunesdb_from_backup(ipod_mount: &Path) -> Result<()>` — errors if backup missing; **opens the backup with libgpod first** (`OwnedDb::open_file`-style parse of the backup path — add a small `parse_check(path) -> Result<()>` helper using `itdb_parse_file`) and errors if it doesn't parse; renames live DB → `iTunesDB.corrupt` (replace-existing); copies backup → `iTunesDB` via `.tmp` + atomic rename (same pattern as `backup_itunesdb`).
