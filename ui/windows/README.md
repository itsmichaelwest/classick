...
Native WinUI 3 / .NET 10 tray app for Classick. Runs in the system tray, talks
to the long-lived `classick.exe --daemon` over a named pipe, and surfaces
device state, sync progress, settings, and a first-run wizard. The daemon in
turn spawns `classick.exe --ipc-mode --apply` subprocesses to do the actual
...
# or
dotnet build Classick.UI.slnx -c Release
```
...
The UI csproj bundles `..\..\..\target\release\classick.exe` plus the libgpod
runtime DLLs as content, so they're copied next to `Classick.UI.exe` at build
time and shipped inside the MSIX. Build the Rust core first or expect a
"core not found" dialog at startup:

```powershell
# From F:\repos\classick\
cargo build --release
```
...
listening on the named pipe, `SpawnDaemon()` looks for `classick.exe` next to
`Classick.UI.exe` or one directory up, then launches it with `--daemon`. The
...
The named-pipe label is `\\.\pipe\classick`, set on the Rust side by
`PIPE_NAME` in `crates/classick/src/daemon/ipc_server.rs` and mirrored on
the UI side by `Classick.UI.Core.AppIdentity`. **These two MUST stay in
...
```
ui\windows\
├── Classick.UI.slnx                  .NET 10 XML solution; x64 + ARM64
├── README.md                         (this file)
│
├── Classick.UI\                      Main WinUI 3 tray app
...
│
├── Classick.UI.Core\                 Pure System.* class library (no WinUI)
...
│                                     (per docs/ipc-protocol.md)
│
└── Classick.UI.Tests\                xUnit on plain net10.0
...
### Why a separate `Classick.UI.Core` project — the WinAppSDK module-init hazard
...
`Classick.UI.Core` is intentionally pure `System.*` code so both the WinUI app
...
still single-truth in `Classick.UI\ViewModels\`, but the test process never
touches the WinAppSDK module initializer.

## Conventions and gotchas

- **Namespace is `Classick_UI` (underscore), not `Classick.UI`.** The csproj
  pins `<RootNamespace>Classick_UI</RootNamespace>`. New files should use
  `namespace Classick_UI;` (and `Classick_UI.Services`, `Classick_UI.Views`,
  etc.).
