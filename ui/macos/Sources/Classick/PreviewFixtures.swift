...
enum PreviewFixtures {
    ...
    static let boardsOfCanada = LibraryArtist(name: "Boards of Canada", albums: [
        LibraryAlbum(name: "Music Has the Right to Children", genre: "Electronic", tracks: 17, bytes: 595_000_000),
        LibraryAlbum(name: "Geogaddi", genre: "Electronic", tracks: 23, bytes: 805_000_000),
    ])
    static let radiohead = LibraryArtist(name: "Radiohead", albums: [
        LibraryAlbum(name: "OK Computer", genre: "Alternative Rock", tracks: 12, bytes: 408_000_000),
        LibraryAlbum(name: "In Rainbows", genre: "Alternative Rock", tracks: 10, bytes: 340_000_000),
    ])
    static let fleetwoodMac = LibraryArtist(name: "Fleetwood Mac", albums: [
        LibraryAlbum(name: "Rumours", genre: "Rock", tracks: 11, bytes: 330_000_000),
    ])
    static let milesDavis = LibraryArtist(name: "Miles Davis", albums: [
        LibraryAlbum(name: "Kind of Blue", genre: "Jazz", tracks: 5, bytes: 225_000_000),
    ])
    static let beyonce = LibraryArtist(name: "Beyoncé", albums: [
    ...
    static let roadTripMix = PlaylistSummary(
        slug: "road-trip-mix", name: "Road Trip Mix", kind: .manual, tracks: 11, bytes: 380_000_000, error: nil)
    static let nightDrive = PlaylistSummary(
        slug: "night-drive", name: "Night Drive", kind: .manual, tracks: 23, bytes: 780_000_000, error: nil)
    static let electronicEssentials = PlaylistSummary(
        slug: "electronic-essentials", name: "Electronic Essentials", kind: .smart, tracks: 40, bytes: 1_400_000_000, error: nil)
    ...
    private static func trackStartLine(current: Int, total: Int, label: String, etaSecs: UInt64?) -> String {
        let etaField = etaSecs.map { ",\"eta_secs\":\($0)" } ?? ""
        return "{\"type\":\"track_start\",\"current\":\(current),\"total\":\(total),\"label\":\"\(label)\"\(etaField)}"
    }
    ...
    private static func baseConnectedModel(
        ...
        let m = AppModel()
        m.apply(.configUpdate(source: musicFolderPath, daemon: daemonSettings(), ipod: pairedIpod))
        m.apply(.deviceConnected(
            serial: pairedIpod.serial, modelLabel: pairedIpod.modelLabel,
            drive: connectedDevice.drive, name: pairedIpod.name))
        m.apply(.statusUpdate(StatusInfo(
        ...
        m.apply(.historyUpdate(entries: historyEntries))
        m.apply(.playlistsUpdate(playlistSummaries))
    ...
    static func connectedOverfullModel() -> AppModel {
        ...
        m.apply(.deviceConfigUpdate(
            serial: pairedIpod.serial,
            selection: SelectionState(mode: .all, rules: []),
            subscriptions: SubscriptionsWire(playlists: [electronicEssentials.slug, roadTripMix.slug, nightDrive.slug]),
            settings: DeviceSettingsWire(autoSync: true, rockboxCompat: false)))
        m.willRequestDevicePreview(serial: pairedIpod.serial)
        m.apply(.devicePreview(devicePreviewOverfull))
        m.apply(.syncEvent(line: finishOverfullLine))
        return m
    }
    ...
    static func notConfiguredModel() -> AppModel {
        let m = AppModel()
        m.apply(.configUpdate(source: musicFolderPath, daemon: daemonSettings(), ipod: nil))
        m.apply(.libraryUpdate(richLibrary))
        m.apply(.deviceConnected(
            serial: pairedIpod.serial, modelLabel: pairedIpod.modelLabel,
            drive: connectedDevice.drive, name: pairedIpod.name))
    ...
    static func scanningModel() -> AppModel {
        ...
        m.apply(.syncEvent(line: trackStartLine(current: 47, total: 91, label: "", etaSecs: nil)))
        return m
    }
    ...
    static func syncingModel() -> AppModel {
        ...
        m.apply(.syncEvent(line: trackStartLine(
            current: 34, total: 91,
    ...
    static func pausedModel() -> AppModel {
        let m = syncingModel()
        m.apply(.syncEvent(line: pausedLine))
        return m
    }
