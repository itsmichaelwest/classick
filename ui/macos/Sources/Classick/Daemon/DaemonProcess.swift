...
/// the `Process` handle so `stop()` can terminate it on quit.
///
/// Runs on the main actor: it's only ever driven from app-lifecycle events
/// (`applicationDidFinishLaunching`/`applicationWillTerminate`), and
...
final class DaemonProcess {
    ...
    func ensureRunning() {
        if Self.socketIsAnswering(path: socketPath) {
            logger.info("daemon already listening on \(self.socketPath, privacy: .public) — attaching, not spawning")
            return
        }

        guard let binaryURL = Self.locateBinary(logger: logger) else {
            logger.fault("could not locate the classick binary in the app bundle or the dev tree — nothing to spawn")
            return
        }
        ...
        do {
            try proc.run()
            process = proc
            logger.info("spawned daemon: \(binaryURL.path, privacy: .public) --daemon (pid \(proc.processIdentifier))")
        } catch {
            logger.fault("failed to spawn \(binaryURL.path, privacy: .public): \(String(describing: error), privacy: .public)")
        }
    }
