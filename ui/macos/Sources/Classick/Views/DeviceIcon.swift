...
struct DeviceIcon: View {
  ...
  private func resolvedIcon() -> NSImage? {
    let key = DeviceIconLogic.cacheKey(serial: serial, drive: drive)
    ...
    let resource =
      DeviceIconLogic.ampResourcesDir
      + "/\(DeviceIconLogic.iconBaseName(modelNum: modelNum)).icns"
    let icon =
      NSImage(contentsOfFile: resource)
      ?? drive.flatMap { path in
...
enum DeviceIconLogic {
  ...
  nonisolated static func parseModelNum(sysInfo: String) -> String? {
    for line in sysInfo.split(whereSeparator: \.isNewline) {
      ...
      guard parts.count == 2,
        parts[0].trimmingCharacters(in: .whitespaces) == "ModelNumStr"
      else { continue }
