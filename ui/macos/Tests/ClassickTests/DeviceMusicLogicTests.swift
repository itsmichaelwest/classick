...
final class DeviceMusicLogicTests: XCTestCase {
  ...
  private let aphex = LibraryArtist(
    name: "Aphex Twin",
    albums: [
  ...
  func testSeed_includeToAll_keepsRulesDormant() {
    ...
    XCTAssertEqual(
      result, current, "rules must stay dormant, not clear, so switching back restores them")
  }
  ...
  func testSeed_allToInclude_usesContentsCanonicalCaseRegardlessOfStaleRuleCase() {
    ...
    let result = DeviceMusicLogic.seededSelection(
      fromDeviceContents: [radiohead], previousMode: .all, newMode: .include,
      current: staleDifferentCase)
    XCTAssertEqual(
      Set(result),
      [
        .album(artist: "Radiohead", album: "Kid A"),
  ...
  func testSyncNowDisabledWhileScanning() {
    XCTAssertTrue(
      DeviceMusicLogic.isSyncNowDisabled(phase: .scanning(current: 1, total: 10), isConnected: true)
    )
  }
  ...
  func testSyncNowEnabledWhenPaused() {
    XCTAssertFalse(
      DeviceMusicLogic.isSyncNowDisabled(phase: .paused(synced: 5, total: 10), isConnected: true))
  }
  ...
  func testCapacityBarNilWithoutStorage() {
    let preview = DevicePreview(
      serial: "serial-a", selectedTracks: 1, selectedBytes: 100, playlistExtraTracks: 0,
  ...
  func testCapacityBarComputesUsedAndProjectedFractions() {
    let preview = DevicePreview(
      ...
      acknowledgedRequestID: "request-a")
    guard
      let bar = DeviceMusicLogic.capacityBar(
        storage: (free: 6_000_000_000, total: 10_000_000_000), preview: preview)
    else {
  ...
  private func makeLibrary(scannedAt: UInt64?, totalTracks: Int, sourceRoot: String? = "/Music")
    -> LibraryInfo
  {
    LibraryInfo(
      sourceRoot: sourceRoot, scannedAtUnixSecs: scannedAt, artists: [], genres: [],
      totalTracks: totalTracks, totalBytes: 0)
  }
  ...
  func testUnresolvedSubscriptionsLineSingularAndPlural() {
    XCTAssertEqual(
      DeviceMusicLogic.unresolvedSubscriptionsLine(["gym"]),
      "1 subscribed playlist couldn't be resolved")
    XCTAssertEqual(
      DeviceMusicLogic.unresolvedSubscriptionsLine(["gym", "chill"]),
      "2 subscribed playlists couldn't be resolved")
  }
}
...
extension DeviceMusicLogicTests {
  ...
  func testEntireToSelectedWithEmptyMemoryRestoresEmpty() {
    ...
    XCTAssertEqual(
      DeviceMusicLogic.seededSelection(
        fromDeviceContents: artists, previousMode: .all, newMode: .include,
        current: [], remembered: []), [],
